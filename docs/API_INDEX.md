...
Quick reference with 2-line descriptions for all API functions.
...
Allocate VM instance and initialize HashLink runtime.
Returns VM handle or NULL on failure.

**`hlffi_error_code hlffi_init(hlffi_vm* vm, int argc, char** argv)`**
Initialize HashLink system with command-line arguments (call once per process).
Sets up GC, registers main thread, and initializes libuv.

**`hlffi_error_code hlffi_load_file(hlffi_vm* vm, const char* path)`**
Load compiled .hl bytecode from file path.
Returns HLFFI_OK on success or error code on failure.

**`hlffi_error_code hlffi_load_memory(hlffi_vm* vm, const void* data, size_t size)`**
Load compiled .hl bytecode from memory buffer.
Returns HLFFI_OK on success or error code on failure.

**`hlffi_error_code hlffi_call_entry(hlffi_vm* vm)`**
Execute Haxe Main.main() entry point.
Initializes globals and runs main function.

**`void hlffi_destroy(hlffi_vm* vm)`**
Free VM instance (call only at process exit).
Releases all HashLink resources.

**`void hlffi_update_stack_top(void* stack_marker)`**
Update GC stack scanning limit for current thread.
Call when switching stacks or in deep recursion.

**`const char* hlffi_get_error(hlffi_vm* vm)`**
Get last error message (static string, do not free).
Returns detailed error description.

**`const char* hlffi_get_error_string(hlffi_error_code code)`**
Convert error code to human-readable string.
Returns static error message for given code.
...
Set integration mode (NON_THREADED or THREADED).
Must call before hlffi_call_entry().

**`hlffi_integration_mode hlffi_get_integration_mode(hlffi_vm* vm)`**
Get current integration mode.
Returns HLFFI_NON_THREADED or HLFFI_THREADED.
...
Process UV loop and Haxe EventLoop for one frame.
Call every frame in NON_THREADED mode (processes timers, async ops).

**`bool hlffi_has_pending_work(hlffi_vm* vm)`**
Check if VM has pending events/timers.
Returns true if hlffi_update() should be called.

**`hlffi_error_code hlffi_process_events(hlffi_vm* vm, hlffi_eventloop_type type)`**
Process specific event loop type (UV_LOOP or HAXE_EVENTLOOP).
Lower-level control than hlffi_update().

**`bool hlffi_has_pending_events(hlffi_vm* vm, hlffi_eventloop_type type)`**
Check if specific event loop has pending work.
Returns true if events are queued for processing.
...
Start dedicated VM thread (THREADED mode only).
Spawns background thread running event loop.

**`hlffi_error_code hlffi_thread_stop(hlffi_vm* vm)`**
Stop VM thread gracefully.
Waits for thread to finish current work.

**`bool hlffi_thread_is_running(hlffi_vm* vm)`**
Check if VM thread is currently running.
Returns true if thread is active.

**`hlffi_error_code hlffi_thread_call_sync(hlffi_vm* vm, hlffi_thread_func func, void* userdata)`**
Execute function on VM thread and wait for completion (blocking).
Returns when function finishes executing.

**`hlffi_error_code hlffi_thread_call_async(hlffi_vm* vm, hlffi_thread_func func, void* userdata, hlffi_async_callback callback, void* callback_data)`**
Execute function on VM thread asynchronously (non-blocking).
Returns immediately, callback invoked when complete.

**`void hlffi_worker_register(void)`**
Register current C thread with HashLink GC.
Must call before accessing HLFFI from worker threads.

**`void hlffi_worker_unregister(void)`**
Unregister current thread from GC.
Must call before worker thread exits.

**`void hlffi_blocking_begin(void)`**
Notify GC that thread is starting external I/O operation.
Allows GC to run while thread is blocked.

**`void hlffi_blocking_end(void)`**
Notify GC that thread returned from external I/O.
Must balance every hlffi_blocking_begin().
...
Enable/disable hot reload support (must call before load).
Requires HashLink 1.12+ for module patching.

**`bool hlffi_is_hot_reload_enabled(hlffi_vm* vm)`**
Check if hot reload is enabled.
Returns true if hot reload was enabled before load.

**`hlffi_error_code hlffi_reload_module(hlffi_vm* vm, const char* path)`**
Reload bytecode from file (updates code, preserves globals).
...
Same as hlffi_reload_module() but from RAM.
...
Callback receives VM and userdata.

**`bool hlffi_check_reload(hlffi_vm* vm)`**
Check if bytecode file changed and reload if needed.
Returns true if reload occurred (automatic file watching).
...
Find type by fully-qualified name (e.g., "Player").
Returns type handle or NULL if not found.

**`hlffi_type_kind hlffi_type_get_kind(hlffi_type* type)`**
Get type category (VOID, I32, F64, OBJ, etc.).
Returns enum value indicating type kind.

**`hlffi_error_code hlffi_list_types(hlffi_vm* vm, hlffi_type_callback callback, void* userdata)`**
Enumerate all types in loaded module.
Calls callback for each type.

**`hlffi_type* hlffi_class_get_super(hlffi_type* type)`**
Get parent class type.
Returns superclass or NULL if no parent.

**`int hlffi_class_get_field_count(hlffi_type* type)`**
Get number of fields in class.
Returns field count or -1 on error.
...
Check if running in JIT mode (vs interpreter).
Returns true if JIT compiler is active.
...
Box C int to Haxe value (temporary, not GC-rooted).
Use immediately or pass to Haxe functions.

**`hlffi_value* hlffi_value_float(hlffi_vm* vm, double value)`**
Box C double to Haxe Float value.
Temporary value, free with hlffi_value_free().

**`hlffi_value* hlffi_value_bool(hlffi_vm* vm, bool value)`**
Box C bool to Haxe Bool value.
Temporary value for immediate use.

**`hlffi_value* hlffi_value_string(hlffi_vm* vm, const char* str)`**
Box C UTF-8 string to Haxe String (converts to UTF-16).
Copies string data, original can be freed.

**`hlffi_value* hlffi_value_null(hlffi_vm* vm)`**
Create null value.
Used for void returns or null parameters.

**`void hlffi_value_free(hlffi_value* value)`**
Free value and remove GC root.
Must call for all non-temporary values.

**`int hlffi_value_as_int(hlffi_value* value, int fallback)`**
Unbox Haxe value to C int.
Returns fallback if value is null or wrong type.

**`double hlffi_value_as_float(hlffi_value* value, double fallback)`**
Unbox Haxe Float to C double.
Returns fallback on error.

**`bool hlffi_value_as_bool(hlffi_value* value, bool fallback)`**
Unbox Haxe Bool to C bool.
Returns fallback on error.

**`char* hlffi_value_as_string(hlffi_value* value)`**
Unbox Haxe String to C UTF-8 string (caller must free()).
Converts UTF-16 to UTF-8, allocates new buffer.
...
Get static field value by name.
...
Returns result value or NULL on error.
...
Create new object instance (GC-rooted, must free).
Calls constructor with given arguments.
...
Set string field from C UTF-8 string.
Returns true on success.
...
Create dynamic array with initial length.
Element type NULL for Dynamic arrays.
...
Returns element value or NULL if out of bounds.
...
Append element to array (grows by 1).
Returns true on success.

**`hlffi_value* hlffi_native_array_new(hlffi_vm* vm, hl_type* element_type, int length)`**
Create NativeArray for zero-copy access.
Fixed size, direct memory access.

**`void* hlffi_native_array_get_ptr(hlffi_value* arr)`**
Get direct pointer to NativeArray data.
Zero-copy access to array memory.

**`hlffi_value* hlffi_array_new_struct(hlffi_vm* vm, hlffi_type* struct_type, int length)`**
Create array of structs (@:struct classes).
Fixed-size, zero-copy struct storage.

**`void* hlffi_array_get_struct(hlffi_value* arr, int index)`**
Get pointer to struct at index (zero-copy).
Returns direct pointer, modify in-place.

**`bool hlffi_array_set_struct(hlffi_vm* vm, hlffi_value* arr, int index, void* struct_ptr, int struct_size)`**
Copy struct into array at index.
Copies struct data from ptr.

**`hlffi_value* hlffi_native_array_new_struct(hlffi_vm* vm, hlffi_type* struct_type, int length)`**
Create NativeArray of structs.
Zero-copy fixed-size struct array.

**`void* hlffi_native_array_get_struct_ptr(hlffi_value* arr)`**
Get pointer to first struct in NativeArray.
Direct pointer to array memory.
...
Create hash map with key/value types.
Use NULL for Dynamic types.
...
Allocate byte buffer (zero-initialized).
Returns bytes object or NULL.

**`hlffi_value* hlffi_bytes_from_data(hlffi_vm* vm, const void* data, int size)`**
Create bytes from C data (copies data).
Returns bytes object containing copy.

**`hlffi_value* hlffi_bytes_from_string(hlffi_vm* vm, const char* str)`**
Create bytes from C string (no null terminator).
Copies string contents.

**`void* hlffi_bytes_get_ptr(hlffi_value* bytes)`**
Get direct pointer to byte buffer.
Zero-copy access for in-place modification.
...
Convert bytes to C string (caller must free()).
Null-terminates and returns copy.
...
Create enum with constructor parameters.
Returns enum value or NULL.

**`bool hlffi_enum_is(hlffi_value* value, int index)`**
Check if enum is specific constructor index.
Returns true if matches.

**`bool hlffi_enum_is_named(hlffi_value* value, const char* name)`**
Check if enum is specific constructor name.
Returns true if matches.
...
Get abstract type name (caller must free()).
Returns name or NULL if not abstract.

**`hlffi_type* hlffi_abstract_find(hlffi_vm* vm, const char* name)`**
Find abstract type by name.
Returns type or NULL if not found.

**`bool hlffi_value_is_abstract(hlffi_value* value)`**
Check if value is abstract type.
Returns true for abstract values.

**`char* hlffi_value_get_abstract_name(hlffi_value* value)`**
Get abstract name of value (caller must free()).
Returns name or NULL.
...
Register C callback callable from Haxe.
Returns true on success (register before load).

**`bool hlffi_register_callback_typed(hlffi_vm* vm, const char* name, hlffi_native_func func, int nargs, hlffi_type** arg_types, hlffi_type* ret_type)`**
Register typed callback (deprecated - use Dynamic).
Returns true on success.

**`hlffi_value* hlffi_get_callback(hlffi_vm* vm, const char* name)`**
Get registered callback function.
Returns callback or NULL if not registered.

**`bool hlffi_unregister_callback(hlffi_vm* vm, const char* name)`**
Remove registered callback.
Returns true if callback was found.

---

## Exception Handling

**`hlffi_call_result hlffi_try_call_static(hlffi_vm* vm, const char* class_name, const char* method_name, int argc, hlffi_value** argv)`**
Call static method with exception safety.
Returns result with value OR exception set.

**`hlffi_call_result hlffi_try_call_method(hlffi_value* obj, const char* method_name, int argc, hlffi_value** argv)`**
Call instance method with exception safety.
Returns result with value OR exception set.

**`char* hlffi_get_exception_message(hlffi_value* exception)`**
Get exception message (caller must free()).
Returns error message string.

**`char* hlffi_get_exception_stack(hlffi_value* exception)`**
Get exception stack trace (caller must free()).
Returns full stack trace.

**`bool hlffi_is_exception(hlffi_value* value)`**
Check if value is exception object.
Returns true for exceptions.

**`bool hlffi_has_exception(hlffi_vm* vm)`**
Check if VM has pending exception.
Returns true if exception occurred.

**`void hlffi_clear_exception(hlffi_vm* vm)`**
Clear pending exception.
Resets exception state.
...
Cache static method for 60x faster calls.
Returns cache handle or NULL on error.
...
Free cached method handle.
Must call to avoid memory leak.

**`hlffi_cached_call* hlffi_cache_instance_method(hlffi_vm* vm, const char* class_name, const char* method_name)`**
Cache instance method (NOT YET IMPLEMENTED).
Returns NULL currently.

**`hlffi_value* hlffi_call_cached_method(hlffi_cached_call* cached, hlffi_value* instance, int argc, hlffi_value** argv)`**
Call cached instance method (NOT YET IMPLEMENTED).
Returns NULL currently.
